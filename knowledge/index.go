...
type Index struct {
	...
	summaries     map[string]string // "<kind>:<id>" -> distilled blockquote summary
}
...
// Summary returns the distilled blockquote summary of a rule/pattern/workflow,
// or "" when its body carries none.
func (i *Index) Summary(kind, id string) string { return i.summaries[kind+":"+id] }
