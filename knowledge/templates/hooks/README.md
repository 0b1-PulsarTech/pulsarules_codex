...
| `post-edit`       | after each Write/Edit (deduped per session by content)                                                     | A file-type-aware checklist of the skills to re-validate against the edited file's extension (only those actually installed). It is also the ONE mode that writes a project file: it removes from the edited file the invisible markers no context can justify (zero-width characters, bidi controls, soft hyphens) and folds exotic spaces to a plain space, then names every removal in its output so the change is visible instead of silent. It refuses any path outside the project, a symlink, an extension other than `.go`/`.md`, anything under `testdata/`, a file over 1 MiB, or invalid UTF-8, and it never rewrites an em dash, an ellipsis or an invisible that may be load-bearing - those are reported by the `typographic-markers` and `text-markers` analyzers instead. |
