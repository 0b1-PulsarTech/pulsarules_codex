...
// knowledge index's own data - router.yaml's baseline and dispatch table,
// plus each skill's triggers - instead of a hardcoded per-extension map.
// Adding a trigger to skills.yaml routes a new skill with no Go change.
type Router struct {
...
// router.yaml always-load baseline for any .go file, any skill whose
// trigger literally names the extension (e.g. database-persistence's
// ".sql", grpc-adapter's ".proto"), and "Any test work"'s skills for a Go
// test file. Returns nil for a nil Router/index or an unmatched extension.
...
//
// simplification: could over-match ext as a prefix (".sql" in ".sqlite");
// acceptable while hand-authored. Upgrade path: exact suffix/word matching.
